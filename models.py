...
class BaselineModelWithRGCN(torch.nn.Module):
  def __init__(self, config, metadata):
    super(BaselineModelWithRGCN, self).__init__()
    ...
    self.convs = torch.nn.ModuleList(
      [HGTConv(self.embed_size_gnn, self.embed_size_gnn, metadata=metadata) for _ in range(3)])
    ###
    self.bns_gnn_hgt = torch.nn.ModuleList([torch.nn.BatchNorm1d(num_features=self.embed_size_gnn) for _ in range(3)])
    self.bns = torch.nn.ModuleList([torch.nn.BatchNorm1d(num_features=self.embed_size) for _ in range(2)])
    ...
    for node_type in metadata[0]:
      self.node_lin[node_type] = [torch.nn.Linear(in_features=300, out_features=self.embed_size_gnn)]
      self.node_lin[node_type].append(
        torch.nn.Linear(in_features=self.embed_size_gnn, out_features=self.embed_size_gnn))
      self.node_lin[node_type] = torch.nn.ModuleList(self.node_lin[node_type]).to(device)
      self.bns_gnn_node = {
        node_type: torch.nn.ModuleList([torch.nn.BatchNorm1d(num_features=self.embed_size_gnn) for _ in range(2)]).to(
          device)
      }

    for node_type in metadata[0]:
      self.node_lin_post[node_type] = [
        torch.nn.Linear(in_features=self.embed_size_gnn, out_features=self.embed_size_gnn)]
      self.node_lin_post[node_type].append(
        torch.nn.Linear(in_features=self.embed_size_gnn, out_features=self.embed_size_gnn))
      self.node_lin_post[node_type] = torch.nn.ModuleList(self.node_lin_post[node_type]).to(device)
      self.bns_gnn_node_post = {
        node_type: torch.nn.ModuleList([torch.nn.BatchNorm1d(num_features=self.embed_size_gnn) for _ in range(2)]).to(
  ...
  def forward(self, ids_sent1, segs_sent1, att_mask_sent1, graph, graph_masking, edge_dict, visualize=False):
    ...
    if num_nodes == 0:
      ...
      for i in range(len(self.convs)):
        ...
        # adapt the convolution parameters if needed
        out = {
          node_type: self.dp_hgt[i](
            self.bns_gnn_hgt[i](self.relu(self.convs[i](x_dict, edge_index_dict, edge_attr_dict=None)[node_type])))
          for node_type, _ in x_dict.items()
        }  # edge_attr_dict=rel_dict)))
        ###
